...
final class Keccak
{
    ...
    private static function keccak64(string $inRaw, int $capacity, int $outputlength, int $suffix, bool $rawOutput): string
    {
        ...
        for ($in_t = 0; $inlen >= $rsiz; $inlen -= $rsiz, $in_t += $rsiz) {
            for ($i = 0; $i < $rsizw; $i++) {
                $t = unpack('V*', mb_substr($inRaw, $i * 8 + $in_t, 8, self::ENCODING));
        ...
        $temp = mb_substr($inRaw, $in_t, $inlen, self::ENCODING);
        $temp = str_pad($temp, $rsiz, "\x0");
    ...
    public static function shake(string $in, int $securityLevel, int $outputLength, bool $rawOutput = false): string
    {
        if (! in_array($securityLevel, [128, 256], true)) {
            throw new Exception('Unsupported Keccak Shake security level.');
        }

        return self::keccak($in, $securityLevel, $outputLength, 0x1f, $rawOutput);
    }
}
